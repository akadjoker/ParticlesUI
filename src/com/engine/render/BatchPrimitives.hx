...
public function fcolor(r:Float, g:Float,b:Float, ?a:Float =0.0)
	...
	{
	//shader.Enable();
	Game.primitiveShader.Enable();
	BlendMode.setBlend(currentBlendMode);

	Game.primitiveShader.setProjMatrix(Game.camera.projMatrix);
	Game.primitiveShader.setViewMatrix(viewMatrix);
    ...
    // GL.uniformMatrix3D(shader.modelViewMatrixUniform, false, viewMatrix);
     ...
     GL.vertexAttribPointer(Game.primitiveShader.vertexAttribute, 3, GL.FLOAT, false, 0, 0);
	 GL.bindBuffer(GL.ARRAY_BUFFER, this.fcolorBuffer);
	 GL.bufferSubData(GL.ARRAY_BUFFER, 0, this.fcolors);
     GL.vertexAttribPointer(Game.primitiveShader.colorAttribute, 4, GL.FLOAT, false, 0, 0);
